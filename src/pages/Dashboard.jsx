...
import { 
    PlayCircle, Settings, ArrowRight, Dices, BarChart3, Flame, Target, ClipboardList, Sparkles 
} from 'lucide-react';
    ...
    return (
        <div className="min-h-screen pb-28 px-4 pt-6 max-w-3xl mx-auto animate-fade-in">
            ...
            <header className="mb-6 flex justify-between items-center">
                ...
                <button onClick={() => navigate('/settings')} className="p-2.5 rounded-full bg-gray-800/50 hover:bg-gray-700 text-gray-400 hover:text-white transition-colors border border-gray-700/50">
                    <Settings size={20} />
                </button>
            </header>
            ...
            <div className="grid grid-cols-2 gap-3 mb-8 h-48">
                <div className="card glass-card p-5 flex flex-col justify-center relative overflow-hidden bg-gradient-to-br from-gray-800/80 to-gray-900/80 border border-white/5">
                    ...
                    <div className="flex items-baseline gap-1 mt-1">
                        <p className="text-6xl font-black text-white tracking-tighter drop-shadow-lg">{currentScore}</p>
                        <span className="text-sm text-gray-500 font-medium">/ 100</span>
                    </div>
                    <div className="mt-4">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${currentScore >= 80 ? 'bg-purple-500/20 text-purple-300 border-purple-500/30' : currentScore >= 60 ? 'bg-blue-500/20 text-blue-300 border-blue-500/30' : 'bg-yellow-500/20 text-yellow-300 border-yellow-500/30'}`}>
                            Grade {currentGrade}
                        </span>
                    </div>
                ...
                <div className="flex flex-col gap-3 h-full">
                    ...
                    <div className="card glass-card flex-1 p-4 flex flex-col justify-center relative bg-gray-800/60 border border-white/5">
                        <div className="flex items-center gap-2 mb-1"><Target size={16} className="text-blue-400" /><span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Goal</span></div>
            ...
            <div className="space-y-4">
                ...
                <button
                    ...
                    className="w-full group relative overflow-hidden rounded-2xl bg-gradient-to-r from-amber-600 to-orange-600 p-5 text-left shadow-lg shadow-orange-900/20 transition-all active:scale-[0.98] tap-target border border-white/10"
                >
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-white mb-0.5">AI無限演習 <span className="text-[10px] bg-white/20 px-2 py-0.5 rounded text-white ml-1">Beta</span></h3>
                                <p className="text-orange-100 text-xs font-medium opacity-90">AIが新しい問題を生成</p>
                            </div>
                        </div>
                ...
                <button onClick={() => navigate('/quiz', { state: { mode: 'random', start: true } })} className="w-full group relative overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 to-indigo-600 p-5 text-left shadow-lg transition-all active:scale-[0.98] tap-target border border-white/10">
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-white/20 rounded-xl backdrop-blur-sm shadow-inner"><Dices size={28} className="text-white" /></div>
                            <div><h3 className="text-xl font-bold text-white mb-0.5">ランダム演習</h3><p className="text-blue-100 text-xs font-medium opacity-90">全範囲から5問出題</p></div>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center transition-colors"><ArrowRight className="text-white w-5 h-5" /></div>
                    </div>
                </button>

                <button onClick={() => navigate('/quiz', { state: { mode: 'category_select' } })} className="w-full group relative overflow-hidden rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 p-5 text-left shadow-lg transition-all active:scale-[0.98] tap-target border border-white/10">
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-white/20 rounded-xl backdrop-blur-sm shadow-inner"><BarChart3 size={28} className="text-white" /></div>
                            <div><h3 className="text-xl font-bold text-white mb-0.5">分野別演習</h3><p className="text-purple-100 text-xs font-medium opacity-90">特定の分野を重点的に学習</p></div>
                        </div>
                        <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center transition-colors"><ArrowRight className="text-white w-5 h-5" /></div>
                    </div>
                </button>

                <button onClick={() => navigate('/quiz', { state: { mode: 'mock', start: true } })} className="w-full group relative overflow-hidden rounded-2xl bg-gradient-to-r from-teal-600 to-emerald-600 p-5 text-left shadow-lg transition-all active:scale-[0.98] tap-target border border-white/10">
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-4">
